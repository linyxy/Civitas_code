...
public class SlidingMenuView extends ViewGroup{
    ...
    private void initWorkspace() {
        ...
        mTouchSlop = ViewConfiguration.get(getContext()).getScaledTouchSlop();//回传一适当对像素量用于触控使用，当手指滑动界面时，
                                                       //大于此值可判断为user滑动，否则为不小心触碰或抖动
    }
    ...
    public void computeScroll() {
    	if (mScroller.computeScrollOffset()) {                   //判断滚动是否结束，返回滚动值
	...
	public void measureViews(int widthMeasureSpec, int heightMeasureSpec){
		...
		View v2 = findViewById(R.id.sliding_body_mysecond);
	    v2.measure(widthMeasureSpec, heightMeasureSpec);
	    	
	    View v3 = findViewById(R.id.sliding_body_square);
	    v3.measure(widthMeasureSpec, heightMeasureSpec);
	    
	    View v4 = findViewById(R.id.sliding_body_street);
	    v4.measure(widthMeasureSpec, heightMeasureSpec);
	    
	    View v5 = findViewById(R.id.sliding_body_market);
	    v5.measure(widthMeasureSpec, heightMeasureSpec);
	}
